...
then
    ...
    failed_check_command () {
        ...
        then
            python3 -m pip --disable-pip-version-check install -r "$__temp_var__file_to_watch"
        fi
        # if python2 exists
        if [ -n "$(command -v "python2")" ]
        then
            python2 -m pip --disable-pip-version-check install -r "$__temp_var__file_to_watch"
        fi
        # if successful
        if [ $? -eq 0 ] 
        then
